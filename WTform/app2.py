...
app.config['SECRET_KEY'] = 'mykey' #本当は乱数の方が良いが、今回は簡易的に設定する
...
class UserForm(Form):
    name = StringField("名前：　", validators = [validate_name2, DataRequired('データを入力してください')], widget = TextArea(), render_kw = {"placeholder": "姓　名"})
    age = IntegerField("年齢：　", validators = [NumberRange(0,100, '正しくない値です')])
    password = PasswordField("パスワード：　", validators = [Length(1,10,"パスワードは１０文字以内で入力してください"),EqualTo('confirm_password','パスワードが一致しません')])
    confirm_password = PasswordField("パスワード再入力: ", )
    birthday = DateField("誕生日：　", format='%Y/%m/%d', render_kw = {"placeholder": "yyyy/mm/dd"})
    gender = RadioField('性別：　', choices = [('man', '男性'), ('wo man', '女性')], default = "man")
    ...
    def validate_name(form, field):#validate(新しいエラーの関数を作成)_(今回エラーを表示させたいフィールドを指定)
        if field.data == 'nanashi':
            raise ValidationError('その名前は使用できません')

    #複数のフィールドが重なっている場合にエラーをチェックしたい（例：誕生日と年齢に矛盾がある時など）
    def validate(self):
        ...
        if 0 <=( today - birthday_of_this_age).days <= 365: #この場合正しい年齢になっている
            return True
        flash("年齢と誕生日が一致していません")
        return False
...
def index():
    ...
    if request.method == "POST" and form.validate():
        #sessionを使うとテンプレート間、もしくは別の関数間で値を受け渡すのが簡単になる
